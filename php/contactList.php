...
class ContactList extends Dbh{
    protected function userExists($email, $id = -1){
        $stmt = $this->connect()->prepare('SELECT * FROM `contacts` WHERE emailAdd = ?');
    ...
    protected function retrieveContacts(){
        $stmt = $this->connect()->prepare('SELECT * FROM contacts ORDER BY lname');
        if(!$stmt->execute()){
            $stmt = NULL;
            return array(
                'message' => 'error'
    ...
    protected function insertContact($fname, $lname, $email, $contNum){
        $stmt = $this->connect()->prepare('INSERT INTO contacts (lname, fname, emailAdd, contactNum) VALUES (?,?,?,?);');
    ...
    protected function updateContact($lname, $fname, $email, $contNum, $id){
        $stmt = $this->connect()->prepare('UPDATE contacts SET lname=?,fname=?,emailAdd=?,contactNum = ? WHERE id = ?;');
